...
# Separa por tres líneas y nombre claro
# Reemplazar la sección `BASE_MISIONES` completa
...
# --- BLOQUE DE CÓDIGO ACTUALIZADO ---
# Separa por tres líneas y nombre claro
# Reemplazar la sección `app.post("/api/mando-integral")` completa
...
async def mando_integral(request: Request):
    ...
    if force_recovery_mission:
        ...
        # Usamos oraculo_manifiesto para la calidez humana inicial
        textos_oraculo_casa = MANIFIESTOS_ORACULO.get(mente, MANIFIESTOS_ORACULO["aburrido"])
        oraculo_manifiesto_seleccionado = random.choice(textos_oraculo_casa) # This is the "calidez humana" for CASA mode
        ...
        return JSONResponse({
            ...
            "oraculo_manifiesto": oraculo_manifiesto_seleccionado, # Return this for initial voice
        ...
        # For SALIR mode, the initial voice instruction comes from the first mission's destino_instruccion
        initial_voice_instruction = ""
        if final_misiones_para_frontend:
            initial_voice_instruction = final_misiones_para_frontend[0].get("destino_instruccion_en" if lang == "en" else "destino_instruccion", "")


        return JSONResponse({
            "DIRECCIONAMIENTO_MASTER": "ACCION_CAMPO",
            "misiones": final_misiones_para_frontend,
            "oraculo_manifiesto": initial_voice_instruction, # Use this for initial voice in engine.js
            "historial_salir_actualizado": historial_salir,
            "forced_recovery": False,
            "legal_notice_es": ADVERTENCIA_LEGAL_ES,
            "legal_notice_en": ADVERTENCIA_EN,
            "drive_prohibited": True
        })
# --- FIN BLOQUE DE CÓDIGO ACTUALIZADO ---
