...
def seleccionar_misiones_casa_inteligente(
    ...
    return resultado
# ==========================================================================================
# OPEN THAN GO: MOTOR DE INVERSIÓN SISTÉMICA Y CAPTURA CONCIENCIAL ABSOLUTA
# ==========================================================================================

# 1. MATRIZ ABSOLUTA DE INPUTS EMOCIONALES DE IDENTIFICACIÓN DEL CLIENTE
...
# 3. CAPA ALGORÍTMICA DE INFRAESTRUCTURA DE USA Y ENTORNO PÚBLICO/ESTATAL (INTERFACES DE HACKEO)
...
# 4. CAPA ANATÓMICA DE AUDITORÍA ORGÁNICA (NECESIDADES PERSONALES NATURALES)
...
DICCIONARIO_SISTEMA_MATRIZ = {
    ...
    "marcas_usa": MATRIZ_CONSUMO_EMPRESAS, "infraestructura_usa": MATRIZ_INFRAESTRUCTURA_ENTORNO,
    "procesos_biologicos": MATRIZ_ELIMINACION_HOMEOSTASIS
}

@app.get("/api/v1/algoritmo-captura")
async def obtener_matriz_captura():
    """Endpoint maestro que expone los inputs del manifiesto al frontend."""
    return {"status": "success", "data": DICCIONARIO_SISTEMA_MATRIZ}

@app.post("/api/v1/algoritmo-procesar-inversion")
async def procesar_inversion_sistemica(datos_captura: dict):
    """
    Recibe los datos del juego (Estado + Marca/Lugar + Zip Code públicos).
    Procesa de manera determinista e instantánea el contraataque biológico de 60 segundos.
    """
    ...
    if estado_actual not in ESTADOS_IDENTIFICACION_CLIENTE:
        return {"status": "error", "message": "Estado emocional no registrado en el manifiesto."}
        
    diagnostico = f"Mapeo Biopsicosocial: El cliente se identifica como [{estado_actual}] consumiendo/habitando [{elemento_sistema}] en Zip Code {zip_code}."
    
    # Matriz Algorítmica de Contraataques (Fase 5)
    instruccion_fisiologica = "Expulsión digital activa. Dirígete a tu entorno físico para ejecutar tu función biológica de eliminación o respiración diafragmática."
    
    # 1. Secuestro de Dopamina Digital (Redes sociales)
    if elemento_sistema in ["TikTok", "Instagram", "Facebook", "YouTube Shorts", "X (Twitter)"]:
        instruccion_fisiologica = (
            "Tu mente está atrapada en el secuestro dopaminérgico digital de esta corporación. "
            "El sistema intenta capitalizar tu vacío. Bloquea el dispositivo ahora mismo. "
            "Ejecuta el Módulo de Ventilación Celular: Inhala hondo, retén 4s y expulsa todo el CO2 residual. "
            "Date cuenta de que TÚ eres el único producto en el que debes invertir en este segundo."
        )
    # 2. Infraestructuras Corporativas e Industriales de USA
    elif any(keyword in elemento_sistema for keyword in ["Corporativa", "Wall Street", "Fábrica", "Industrial"]):
        instruccion_fisiologica = (
            "La máquina de producción estadounidense está consumiendo tu biología en silencio. "
            "Rompe el automatismo laboral de inmediato. Levántate de ese asiento de forma voluntaria. "
            "Dirígete al baño de esta infraestructura. Ejecuta tu proceso natural de eliminación (Orinar/Defecar) "
            "con absoluta presencia mental. Mientras la empresa factura millones, tú recuperas la soberanía "
            "de tus órganos. Activa tu sistema linfático caminando 60 segundos por el pasillo."
        )
    # 3. Entornos del Sector Público, Salud e Institucional
    elif any(keyword in elemento_sistema for keyword in ["Hospital", "Clínica", "DMV", "Juzgado", "Gobierno"]):
        instruccion_fisiologica = (
            "Este entorno institucional está inyectando altos niveles de cortisol en tu sistema nervioso. "
            "Despega los ojos de la pantalla. Identifica a tres personas desconocidas en esta sala de espera de USA. "
            "Míralas con compasión; comprende que sufren el mismo engranaje de monotonía y agobio. "
            "Envíales un deseo mental silencioso de paz y salud. Has saboteado el dolor de esta infraestructura."
        )
    # 4. Elementos Recreativos y No Humanos (Naturaleza Libre)
    elif any(keyword in elemento_sistema for keyword in ["Playa", "Parque", "Bosque", "Sol", "Luna", "Aire"]):
        instruccion_fisiologica = (
            "Estás rodeado de una infraestructura natural que costó millones mantener y la estás ignorando por una pantalla. "
            "Suelta el teléfono. Permite que tus pies toquen la tierra o la arena libre. Siente el viento "
            "forzando la homeostasis térmica de tu piel. Clava tu mirada en las nubes, en el sol que oxigena tus "
            "células gratis o en la luna. Pásale la factura al paisaje: absorbe la inmensidad del cielo por 60 segundos."
        )
    ...
    return {
        ...
        "ejecucion_fisiologica_obligatoria": instruccion,
