...
# FILTRO INTERCEPTOR SIN TRABAJO NI STAFFING
    ...
    for i,info in enumerate(tres_mejores):
        if any(p in desahogo for p in palabras_criticas) or opcion_usuario=="MANDO_LIBRE":
            ...
            else: # MAPS (when forced by MANDO_LIBRE or critical words)
...
# FÓRMULA GEOGRÁFICA UNIVERSAL FIJA ORIGINAL RESTAURADA SIN RECORTE NI ALTERACIONES
        ...
        if gps_query:
            if link_base.startswith("http"):
                ...
                link_google_maps_vivo=f"https://www.google.com/maps/search/{gps_query}+in+{anclaje_geografico}".replace(" ","+")
        else:
            link_google_maps_vivo=link_base.replace(" ","+") # This implies link_base might already be a full URL like spotify or youtube
