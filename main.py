# OPEN THAN GO SYSTEM - Contextual Wellbeing Routing Engine (CWRE) V.6.0.1
    ...
    async def mando_integral(request: Request):
        ...
        if budget == "0":
            ...
            precio_real = "GASTO: Libre. El dinero es tu herramienta de escape hoy." if lang == "es" else "COST: Open. Money is your escape tool today."
        ...
        if gps_query:
            if link_base.startswith("http") and "google.com/maps" in link_base: # Ensure it's a map link for geo
                ...
                final_link = f"{link_base}{gps_query}+in+{anclaje_geografico}".replace(" ", "+") if "staffing" in gps_query else link_base.replace(" ", "+")
        else:
