...
# File: main.py - SECCIÓN 1 DE 2 (Backend Core Logic)
...
# Asegura que el directorio 'static' exista para servir archivos estáticos
if not os.path.exists("static"):
    os.makedirs("static")

# Monta el directorio 'static' para que FastAPI pueda servir archivos como engine.js y session.html
...
# Ruta raíz para servir el archivo HTML principal de la aplicación
@app.get("/")
async def index():
...
# Company: May Roga LLC
# File: main.py - SECCIÓN 2 DE 2 (API Endpoint Logic)

# Endpoint principal para el "Mando Integral" que procesa las solicitudes del frontend
@app.post("/api/mando-integral")
async def mando_integral(request: Request):
    payload = await request.json()
    opcion_usuario = str(payload.get("modo", "")).strip().upper()
    zip_code = str(payload.get("zip", "")).strip()
    estado = str(payload.get("estado", "FL")).strip() # Por defecto 'FL' si no se especifica
    region = str(payload.get("region", "")).strip()
    mente = str(payload.get("mente", "agotado")).lower() # Estado mental (ej. agotado, estresado, aburrido)
    budget = str(payload.get("budget", "0")) # Presupuesto (0: cero gasto, 1: bajo, >1: libre)
    perfil = str(payload.get("perfil", "solo")).lower() # Acompañamiento (solo, familia, accesible)
    desahogo = str(payload.get("desahogo", "")).lower() # Texto libre del usuario sobre su problema
    lang = str(payload.get("lang", "es")).lower() # Idioma de la interfaz

    # Captura las métricas de clics acumuladas localmente en engine.js para las 19 necesidades
    # Este perfil dinámico es crucial para el CWRE
    ...
    # Selecciona las opciones de salida basadas en el estado mental del usuario
    ...
    info = None
    if opciones_salir: # Asegura que haya opciones antes de procesar
        if len(opciones_salir) >= 1:
            mejor_score = -1
           
            # Iterar sobre cada opción de salida para encontrar la que mejor se ajusta al perfil del usuario
            for opc in opciones_salir:
                ...
                for necesidad, peso_usuario in perfil_local.items():
                    # Multiplica el peso de la necesidad en el perfil del usuario por el valor de esa necesidad en el lugar
                    # Por defecto, si una necesidad no está en vector_lugar, se asume un valor neutro (ej. 50)
                ...
                if score_coincidencia > mejor_score:
                    ...
                    info = opc
        else: # Si hay opciones pero solo una, tómala directamente
            info = opciones_salir[0]
    else: # Si no hay opciones de salir para ese 'mente', usa una opción por defecto
        info = random.choice(BASE_MISIONES["SALIR"]["aburrido"]) # Fallback seguro
    
    if not info: # Doble chequeo si info aún es None (muy poco probable con los fallbacks)
    ...
    canal_multimedia = random.choice(["SPOTIFY", "YOUTUBE", "MAPS"]) # Selecciona un canal aleatorio si se activa el interceptor
    ...
    if any(p in desahogo for p in palabras_criticas):
        ...
        if lang == "en":
            ...
            # Se usa .get(key, default) para asegurar una guía, incluso si el título no está explícitamente traducido.
    ...
    if gps_query:
        ...
        else: # Esto cubre casos como spotify_audio que no necesitan query de GPS
    ...
    return JSONResponse({
        ...
        "token_entorno": info.get("titulo", "general") # Inyecta la firma para que engine.js sume al perfil dinámico
    })

# Punto de entrada para ejecutar la aplicación con Uvicorn
if __name__ == "__main__":
    # Obtiene el puerto de las variables de entorno o usa 8000 por defecto
