...
async def mando_integral(request: Request):
    ...
    lang = str(payload.get("lang", "es")).lower()
        # --- ENRUTAMIENTO QUIRÚRGICO DE CANAL ESPECIAL (VETERANOS, MAYOR, GOBIERNO) ---
    perfil_especial_oculto = str(payload.get("perfil_especial", "")).lower()
    if not perfil_especial_oculto and perfil_tipo in ["veterano", "mayor", "gobierno"]:
        perfil_especial_oculto = perfil_tipo

    if perfil_especial_oculto in ["veterano", "mayor", "gobierno"]:
        import random
        modo_actual = str(payload.get("modo", "SALIR")).upper()
        if modo_actual not in ["SALIR", "CASA"]:
            modo_actual = "SALIR"
            
        # Extrae la lista comprimida directamente del diccionario unificado que acabas de fusionar
        # Si tu diccionario de misiones principal se llama MISIONES_DATA o de otra forma, cambia 'MISIONES' por ese nombre
        lista_misiones = MISIONES_CATEGORIAS_ESPECIALES[perfil_especial_oculto][modo_actual]
...
# ==========================================================================================
# BANCO DE DATOS BIOCUANTIFICADOS (MISIONES EXCLUSIVAS CANAL ESPECIAL)
# ==========================================================================================
MISIONES_CATEGORIAS_ESPECIALES = {
    ...
    "gobierno": {
        "SALIR": [
            ...
            {"id": "g_s5", "titulo": "Pausa de Oficina", "descripcion": "Aléjate de teclados y ruidos. Quédate de pie en un rincón pacífico escuchando el entorno.", "diagnostico_sintoma": "Alivio del estrés acústico institucional.", "instruccion_fisiologica": "Siente el espacio open. Pecho expandido y libre."}
        ],
        "CASA": [
            {"id": "g_c1", "titulo": "Reset Óptico", "descripcion": "Bloquea el monitor. Cierra los ojos y cúbrelos suavemente con tus palmas 2 minutos en sombra.", "diagnostico_sintoma": "Alivio de fatiga visual y reducción del agobio mental.", "instruccion_fisiologica": "Mundo digital apagado. Descanso visual en paz."},
