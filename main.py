...
async def mando_integral(request: Request):
    ...
    codigo_postal = str(p.get("zip", "")).strip()
    
    # FÓRMULA GEOGRÁFICA UNIVERSAL DINÁMICA: Absorbe cualquier código de USA introducido
    anclaje_geografico = codigo_postal if codigo_postal else "United+States"
    ...
    # Tu lógica original intacta de selección aleatoria por estado mental
    info = random.choice(BASE_MISIONES["SALIR"].get(mente, BASE_MISIONES["SALIR"]["aburrido"]))

    # SECUESTRO BIG TECH: Interceptamos las marcas reales que usan tus datos para encerrarte en el consumo
    marcas_bucle = ["trabajo", "biles", "deudas", "dinero", "amazon", "walmart", "costco", "fresco", "tienda", "comprar"]
    
    if any(pal in desahogo for pal in marcas_bucle):
        guia = "DESTINO: Staffing. QUÉ HACER: Ve directo con ID. CUÁNDO: Ya. PARA QUÉ: Ganar dinero y romper el bucle del consumo."
    ...
    return JSONResponse({
        ...
        "destino_coordenadas_gps": f"https://google.com{gps}+in+{anclaje_geografico}"
    })
