...
const KERNEL = {
    timerInaccion: null,        // Temporizador para detectar inactividad en el oráculo
    timerClinico: null,         // Temporizador para el reloj de respiración profunda
    temporizadorCascada: null,  // Temporizador para el efecto de cascada de preguntas
    timeLeft: 600,              // Tiempo restante en el reloj clínico (10 minutos)
    isLocked: false,            // Bandera para evitar múltiples envíos de peticiones
    idiomaActual: 'es',         // Idioma actual de la interfaz
    pasosMisiones: [],          // Array para almacenar misiones en modo CASA
    indiceMision: 0,            // Índice de la misión actual en modo CASA
    datosLugarGlobal: null,     // Datos de la acción de campo recibidos del backend
    tipoEscapeGlobal: "",       // Tipo de escape (INTERVENCION_DOMESTICA o ACCION_CAMPO)
   
    // Variables de Control de Tiempo e Impaciencia SOLDADAS para el reloj clínico
    relojRealSegundos: 600,     // Conteo de segundos real para el reloj clínico (sin adelantos)
    contadorToques: 0,          // Cuenta cuántas veces el usuario ha tocado el círculo de respiración
    secuenciaAdelantos: [5, 7, 9, 10, 14, 16, 17, 19, 21, 5], // Segundos de adelanto por toque

    // ARQUITECTURA CONVERSACIONAL SECUENCIAL para el oráculo de preguntas
    bloqueActual: 0,            // Índice del bloque de preguntas actual
    conteoInaccion: 0,          // Conteo para el monitoreo de inactividad
    indicePreguntaCascada: 0,   // Índice de la pregunta actual en el efecto cascada
   
    // CATÁLOGO COMPLETO DE PREGUNTAS DEL ORÁCULO
    CATALOGO_PREGUNTAS: [
    ...
    // Recupera o inicializa el perfil dinámico del usuario desde localStorage
    obtenerPerfilLocal() {
        let perfilRaw = localStorage.getItem("otg_perfil_dinamico");
        if (!perfilRaw) {
            // Perfil inicial con valores neutros para todas las necesidades
            const perfilInicial = {
                ...
                "indicador_ansiedad": 0 // Un indicador para rastrear la ansiedad
            };
            localStorage.setItem("otg_perfil_dinamico", JSON.stringify(perfilInicial));
            return perfilInicial;
        }
        try {
            // Asegura que el perfil es un objeto JSON
            return typeof perfilRaw === "string" ? JSON.parse(perfilRaw) : perfilRaw;
        } catch (e) {
            // Fallback en caso de error de parseo (debería ser raro)
            return { "movimiento": 50, "naturaleza": 50, "silencio": 50 };
        }
    },

    // Inicializa el estado del KERNEL al cargar la página
    init() {
        this.bloqueActual = parseInt(localStorage.getItem("otg_bloque_secuencial")) || 0;
    },

    // Activa la interfaz principal después de la pantalla de bienvenida
    despertarInicial() {
    ...
    // Inyecta las preguntas del oráculo en la interfaz
    inyectarBloquePreguntas() {
        const grid = document.getElementById('contenedor-preguntas-oraculo');
        if (!grid) return;
       
        clearInterval(this.temporizadorCascada); // Detiene cualquier cascada anterior
        grid.innerHTML = ""; // Limpia el contenedor de preguntas
        this.indicePreguntaCascada = 0;
       
        let inicioIdx = this.bloqueActual * 6; // Calcula el índice de inicio del bloque actual
        // Si se han mostrado todos los bloques, reinicia al primer bloque
        ...
        // Inyecta 6 preguntas del bloque actual
        for (let i = 0; i < 6; i++) {
        ...
        // Estiliza el botón de "Mando Libre" como inactivo inicialmente
        ...
        if (btnLibre) {
            ...
            btnLibre.onclick = () => { // Deshabilita el click inicial
                this.hablar(this.idiomaActual === 'es' ? "Por favor, espera a que el mando libre se active." : "Please wait for free control to activate.");
            };
        }
    ...
    // Inicia el efecto de cascada para mostrar y hablar las preguntas
    iniciarEfectoCascada() {
        this.indicePreguntaCascada = 0;
       
        // Inicia el temporizador que muestra y habla las preguntas una por una
        this.temporizadorCascada = setInterval(() => {
            ...
            if (botonParaEliminar) {
                ...
                if (siguienteBoton) {
                    let textoLimpio = siguienteBoton.innerText.substring(3); // Elimina el número de la pregunta
                    this.hablar(textoLimpio);
                } else if (this.indicePreguntaCascada === 5) { // Si es la última pregunta del bloque
                    this.hablar(this.idiomaActual === 'es' ? "Si ninguna te describe, puedes usar el mando libre de abajo." : "If none describe you, you can use the free control below.");
                }
                this.indicePreguntaCascada++;
            } else {
                clearInterval(this.temporizadorCascada); // Detiene la cascada cuando no hay más botones
                this.liberarCajonEscrituraLibre(); // Activa el cajón de escritura libre
            }
        }, 8000); // 8 segundos exactos por pregunta antes de desvanecer
    },

    // Libera el cajón de escritura libre para que el usuario pueda escribir su problema
    liberarCajonEscrituraLibre() {
        ...
        if (textarea) textarea.focus(); // Pone el foco en el textarea

        if (btnLibre) {
            // Estiliza el botón de "Mando Libre" como activo
            btnLibre.style.background = "var(--green-action)";
            btnLibre.style.color = "#fff";
            btnLibre.style.borderColor = "#4caf50";
            btnLibre.onclick = () => { // Asigna la acción de envío al botón
                let textoEscrito = textarea.value.trim();
                if (textoEscrito.length > 3) { // Requiere al menos 4 caracteres para enviar
    ...
    // Monitorea la inactividad del usuario en el oráculo para avanzar bloques o pausar
    iniciarMonitoreoInaccion() {
        clearInterval(this.timerInaccion);
        this.conteoInaccion = 0;
        this.timerInaccion = setInterval(() => {
            this.conteoInaccion++;
            if (this.conteoInaccion === 4 || this.conteoInaccion === 8) { // Avanza de bloque después de cierto tiempo de inactividad
            ...
            } else if (this.conteoInaccion >= 12) { // Pausa el monitoreo si la inactividad es muy prolongada
                clearInterval(this.timerInaccion);
                clearInterval(this.temporizadorCascada);
        ...
        }, 12000); // Se verifica cada 12 segundos
    },

    // Reacciona a la selección de una pregunta o texto libre, y envía la petición al backend
    reaccionarPreguntaSeleccionada(textoPregunta) {
        clearInterval(this.timerInaccion);      // Detiene monitoreo de inactividad
        clearInterval(this.temporizadorCascada); // Detiene el efecto cascada
        this.bloqueActual++; // Avanza al siguiente bloque de preguntas
        localStorage.setItem("otg_bloque_secuencial", this.bloqueActual); // Guarda el bloque actual
       
        // Asigna el valor al input oculto antes de la petición Fetch
        document.getElementById('inp-text-invisible').value = textoPregunta;
        this.ejecutar(); // Ejecuta la lógica principal
    },

    // Función para sintetizar voz (Text-to-Speech)
    hablar(texto) {
        if (!texto) return;
        window.speechSynthesis.cancel(); // Cancela cualquier habla anterior para evitar superposiciones
        let fx = texto.replace(/OPEN THAN GO/gi, "OPEN DAN GO").replace(/<[^>]*>/g, ''); // Limpia y ajusta el texto
        const msg = new SpeechSynthesisUtterance(fx);
        msg.lang = 'es-US'; // Voz fija siempre en español para estabilidad nativa y consistencia
        msg.rate = 1.20; // Velocidad de habla
    ...
    cambiarIdioma(lang) {
        ...
        // SINCRO DE BOTÓN DE INGLÉS REPARADO AL 100% (y español)
        const t = {
    ...
    // Ejecuta la petición al backend para obtener la misión o acción de campo
    async ejecutar() {
        if (this.isLocked) return; // Evita envíos múltiples
        this.isLocked = true;

        // Construye el payload con los datos del formulario y el perfil local
        const payload = {
            ...
            perfil_local: this.obtenerPerfilLocal() // Envía el perfil dinámico acumulado
        };

        const container = document.getElementById('wrapper-interactive');
        document.getElementById('wrapper-form').classList.add('hidden'); // Oculta el formulario
        // Muestra un mensaje de conexión mientras espera la respuesta
        ...
        try {
            ...
            // Si es modo CASA, carga las misiones; si no, las vacía (para modo SALIR)
            if (this.tipoEscapeGlobal === "INTERVENCION_DOMESTICA") {
                this.pasosMisiones = data.misiones.slice(0, 3); // Toma solo las primeras 3 misiones
            } else {
                this.pasosMisiones = [];
            }
            this.procesarFlujoSecuencial(container); // Inicia el flujo secuencial de la respuesta
        } catch (error) {
            console.error("Error al conectar con el backend:", error);
            alert(this.idiomaActual === 'es' ? "Error de conexión o datos inválidos. Por favor, inténtalo de nuevo." : "Connection error or invalid data. Please try again.");
            document.getElementById('wrapper-form').classList.remove('hidden');
    ...
    // Procesa y muestra la interfaz según el tipo de direccionamiento
    procesarFlujoSecuencial(container) {
        clearInterval(this.timerClinico); // Asegura que no haya un timer clínico activo
        window.speechSynthesis.cancel();  // Cancela cualquier habla previa
        ...
        // Lógica para el modo "ACCIÓN DE CAMPO" (SALIR)
        if (this.tipoEscapeGlobal === "ACCION_CAMPO") {
            if (this.datosLugarGlobal) {
                ...
                let retencion = 35; // Temporizador para escuchar la guía antes de abrir el enlace
                ...
                this.timerClinico = setInterval(() => {
                    ...
                    if (retencion <= 0) {
                        ...
                        if (btnCount) btnCount.style.display = 'none'; // Oculta el botón de cuenta regresiva
                        if (btnGps) {
                            btnGps.classList.remove('hidden'); // Muestra el botón para abrir GPS/Canal
                            btnGps.onclick = () => {
                                try {
                                    // Actualiza el perfil dinámico basado en el 'token_entorno' del destino
                                    ...
                                    if (perfil) {
                                        ...
                                        // Podrías añadir más lógica para otras necesidades aquí
                                        localStorage.setItem("otg_perfil_dinamico", JSON.stringify(perfil));
                                    }
                                } catch (e) { console.error("Error al actualizar perfil local:", e); }
                                window.open(this.datosLugarGlobal.destino_coordenadas_gps, '_blank'); // Abre el enlace
                                KERNEL.destruirYReiniciar(); // Reinicia la aplicación después de la acción
                            };
                        }
                    }
                }, 1000);
        ...
        // Lógica para el modo "INTERVENCIÓN DOMÉSTICA" (CASA)
        // Si no quedan misiones, inicia el reloj clínico de respiración
        if (this.indiceMision >= this.pasosMisiones.length) {
            this.iniciarRelojClinicoCasa(container, t);
            return;
        }

        // Muestra la misión actual
        ...
        this.hablar(paso.titulo + " . " + paso.descripcion); // Guía de voz para la misión
        document.getElementById('btn-next').onclick = () => this.avanzarPaso(); // Botón para avanzar de misión
    },

    // Inicia el reloj clínico de 10 minutos con ejercicio de respiración y audios biopreventivos
    iniciarRelojClinicoCasa(container, t) {
        clearInterval(this.timerClinico); // Asegura que no haya un timer anterior activo
        window.speechSynthesis.cancel();  // Cancela cualquier habla previa
        ...
        // Inyecta la interfaz del círculo de respiración
        container.innerHTML = `
        ...
        this.timeLeft = 600; // Reinicia el tiempo restante para la visualización
        this.relojRealSegundos = 600; // Reinicia el contador de segundos real
        this.contadorToques = 0; // Reinicia el contador de toques
        ...
        // Maneja el clic en el círculo de respiración para adelantar el tiempo
        if (circleElement) {
            circleElement.onclick = () => {
                if (this.contadorToques < this.secuenciaAdelantos.length) { // Limita los toques
                    let adelantoSegundos = this.secuenciaAdelantos[this.contadorToques];
                    this.timeLeft = Math.max(this.timeLeft - adelantoSegundos, 0); // Reduce el tiempo
                    this.contadorToques++;
                    try {
                        // Incrementa el "indicador_ansiedad" en el perfil local con cada toque
                    ...
                    if (timerDiv) {
                        timerDiv.innerText = `${m}:${s.toString().padStart(2, '0')}`; // Actualiza el tiempo visible
                    }
                }
            };
        }

        // Inicia el temporizador principal del reloj clínico
        this.timerClinico = setInterval(() => {
            this.relojRealSegundos--; // El reloj real siempre decrementa
            if (this.timeLeft > 0) this.timeLeft--; // El tiempo visible solo decrementa si no ha llegado a cero
           
            let m = Math.floor(this.timeLeft / 60);
            let s = this.timeLeft % 60;
            if (timerDiv) timerDiv.innerText = `${m}:${s.toString().padStart(2, '0')}`; // Actualiza el tiempo en pantalla
           
            // Cambia el texto INHALA/EXHALA y su color según el ciclo de respiración
            if (pulmonDiv) {
                let ciclo = this.relojRealSegundos % 8; // Ciclo de 8 segundos (4s inhala, 4s exhala)
                if (ciclo >= 4) {
                    pulmonDiv.innerText = t.inspira.toUpperCase();
                    pulmonDiv.style.color = "#00bcd4"; // Azul para inhalar
                } else {
                    pulmonDiv.innerText = t.expira.toUpperCase();
                    pulmonDiv.style.color = "#d84315"; // Naranja para exhalar
                }
            }

            // Reproduce audios biopreventivos secuenciales cada 20 segundos
            if (this.relojRealSegundos < 600 && this.relojRealSegundos % 20 === 0) {
                ...
                if (recordatorioTexto) {
                    window.speechSynthesis.cancel(); // Cancela para que no se superpongan
            ...
            // Finaliza el protocolo cuando el reloj real llega a cero
            if (this.relojRealSegundos <= 0) {
                clearInterval(this.timerClinico);
                window.speechSynthesis.cancel();
                if (circleElement) {
                    circleElement.style.animation = "none"; // Detiene la animación del círculo
                    circleElement.style.transform = "scale(1)";
                }
                this.hablar(t.fin);
                alert(t.fin);
                this.destruirYReiniciar(); // Reinicia la aplicación
            }
        }, 1000); // Se ejecuta cada segundo
    },

    // Avanza a la siguiente misión en el modo CASA
    avanzarPaso() {
        this.indiceMision++;
        const container = document.getElementById('wrapper-interactive');
    ...
    // Reinicia completamente la aplicación y borra el estado de la sesión
    destruirYReiniciar() {
        clearInterval(this.timerInaccion);
        clearInterval(this.timerClinico);
        clearInterval(this.temporizadorCascada);
        ...
        this.indiceMision = 0;
        this.isLocked = false;
        // sessionStorage.clear(); // Opcional: limpiar sessionStorage si se usa. localStorage se mantiene para el perfil dinámico.
        location.reload(); // Recarga la página para un reinicio limpio
    }
};

// Se inicializa el KERNEL cuando el DOM está completamente cargado
