// OPEN THAN GO SYSTEM - Kernel Somatic Voice Engine V.6.0.1
    ...
    //==========================================================================================
    // KERNEL INTEGRADO V3 (PARTE 1) - OTG_SENSORIAL como pre-procesador de KERNEL
    //==========================================================================================
    (function() {
        // Define OTG_SENSORIAL first, as KERNEL.despertarInicial will call it
        window.OTG_SENSORIAL = {
            marcas: [
                "Walmart", "Amazon", "YouTube", "Spotify", "Ross Dress for Less", // Marcas solicitadas
                "TikTok", "Instagram", "Netflix", "Uber", "Lyft", "American", "Delta", "JetBlue", "Southwest", "Avianca", "LATAM", "Aeromexico",
                "Copa", "Volaris", "WesternUnion", "Zelle", "Temu",
                ...
                "Wendy's", "Chick fil A", "Nike", "Adidas",
            ...
            urls: { // URLs para enlazar con las marcas
                ...
                "Copa": "https://copaair.com", // Corrected URL
            ...
            // NEW: Brand-specific questions
            brandSpecificQuestions: {
                "Walmart": [
                    {es: "¿Qué sensación te genera el entorno de consumo masivo de Walmart?", en: "What feeling does Walmart's mass consumption environment generate in you?"},
                    {es: "¿Buscas en Walmart un escape a la rutina o una solución a una necesidad específica?", en: "Are you looking for an escape from routine or a solution to a specific need at Walmart?"},
                    {es: "¿Cómo te sientes al pasar por los pasillos de Walmart: agobiado, tranquilo, o indiferente?", en: "How do you feel walking through Walmart's aisles: overwhelmed, calm, or indifferent?"}
                ],
                "Amazon": [
                    {es: "¿La inmediatez de Amazon te alivia o te genera más prisa y expectativas?", en: "Does Amazon's immediacy relieve you or create more rush and expectations?"},
                    {es: "¿Qué vacío intentas llenar al buscar productos en Amazon?", en: "What void are you trying to fill by searching for products on Amazon?"},
                    {es: "¿Sientes que Amazon es una herramienta útil o una distracción constante en tu día?", en: "Do you feel Amazon is a useful tool or a constant distraction in your day?"}
                ],
                "YouTube": [
                    {es: "¿Qué tipo de contenido buscas en YouTube para encontrar calma o distracción?", en: "What kind of content do you look for on YouTube to find calm or distraction?"},
                    {es: "¿Sientes que YouTube te ayuda a aprender o te atrapa en un bucle de consumo pasivo?", en: "Do you feel YouTube helps you learn or traps you in a loop of passive consumption?"},
                    {es: "¿Cómo afecta tu estado mental la cantidad de tiempo que pasas en YouTube?", en: "How does the amount of time you spend on YouTube affect your mental state?"}
                ],
                "Spotify": [
                    {es: "¿Qué emociones buscas evocar con la música que escuchas en Spotify?", en: "What emotions do you seek to evoke with the music you listen to on Spotify?"},
                    {es: "¿Utilizas Spotify para concentrarte, relajarte, o para evadirte de tus pensamientos?", en: "Do you use Spotify to concentrate, relax, or to escape your thoughts?"},
                    {es: "¿Sientes que la música de Spotify te conecta con tu interior o te aísla del entorno?", en: "Do you feel Spotify's music connects you with your inner self or isolates you from your surroundings?"}
                ],
                "Ross Dress for Less": [
                    {es: "¿La búsqueda de ofertas en Ross te genera emoción o agotamiento?", en: "Does the hunt for deals at Ross excite you or exhaust you?"},
                    {es: "¿Qué necesidad subyacente buscas satisfacer al comprar en tiendas como Ross?", en: "What underlying need are you trying to satisfy by shopping at stores like Ross?"},
                    {es: "¿Cómo influye el ambiente de Ross (multitud, ruido, desorden) en tu estado de ánimo?", en: "How does Ross's environment (crowds, noise, clutter) affect your mood?"}
                ],
                // Generic questions for other brands not explicitly listed
                "GENERIC": [
                    {es: "¿Qué expectativa tienes al interactuar con esta marca/servicio hoy?", en: "What expectation do you have interacting with this brand/service today?"},
                    {es: "¿Cómo crees que esta marca/servicio impacta tu bienestar mental?", en: "How do you think this brand/service impacts your mental well-being?"},
                    {es: "¿Hay alguna frustración o alegría recurrente asociada a esta marca/servicio?", en: "Is there any recurring frustration or joy associated with this brand/service?"}
                ]
            },
            seleccionadas: [], // To store selected brands
            
            init() {
                this.inyectarMetasYEstilos(); // Inyecta meta tags y estilos globales
                this.modificarBienvenida(); // Ajusta la pantalla de bienvenida inicial
                this.crearEstructurasFlotantes(); // Crea elementos flotantes como el botón de apagado
            },
            ...
            inyectarMetasYEstilos() {
                ...
                // Styles are now injected into session.html, no need to inject here
            },

            modificarBienvenida() {
                let pb = document.getElementById("pantalla-bienvenida");
                if (!pb) return;
                // The HTML content for the welcome screen is now directly in session.html
                // We only need to adjust dynamic text if necessary, but current session.html covers this
            },

            crearEstructurasFlotantes() {
                // The otg-btn-power and otg-oasis-entretenimiento are now directly in session.html
                // This function ensures they are available in the DOM
                let powerBtn = document.getElementById("otg-btn-power");
                if (powerBtn) powerBtn.onclick = () => this.apagarSistemaTotal();

                let oasisDiv = document.getElementById("otg-oasis-entretenimiento");
                // Nothing else needed here as they are already in the HTML
            },

            interceptarBotonStart() {
                // No need for a 15-minute timer here, KERNEL handles overall session timeouts.
                // Just open the company selection screen.
                this.abrirOasisOcio();
            },
            ...
            abrirOasisOcio() {
                ...
                // Shuffle brands to show different options each time
                this.marcas.sort(() => Math.random() - .5);
               
                let zip = document.getElementById("inp-zip") ? document.getElementById("inp-zip").value.trim() : "";
                let txtUsa = KERNEL.idiomaActual === 'es' ? (zip ? `Opciones disponibles para el Código Postal ${zip}` : "Personaliza tu experiencia") : (zip ? `Options available for Zip Code ${zip}` : "Personalize your experience");
                let continueBtnText = KERNEL.idiomaActual === 'es' ? "Continuar →" : "Continue →";
                let selectServiceText = KERNEL.idiomaActual === 'es' ? "Selecciona el servicio que mejor representa lo que deseas hacer en este momento." : "Select the service that best represents what you wish to do right now.";
                let initialWellnessText = KERNEL.idiomaActual === 'es' ? "Bienestar Inicial" : "Initial Wellness";
                let customizeExperienceText = KERNEL.idiomaActual === 'es' ? "PERSONALIZA TU EXPERIENCIA" : "CUSTOMIZE YOUR EXPERIENCE";
                let approxTimeText = KERNEL.idiomaActual === 'es' ? ". Tiempo aproximado: 1 minuto." : ". Approximate time: 1 minute.";
                    ...
                    <div style="max-width:460px;margin:0 auto;padding-top:5px;">
                        <div style="text-align:center;margin-bottom:15px;">
                            ...
                            <h4 style="color: #00bcd4; font-weight: 900; margin: 8px 0 3px; font-size: 1.15rem;">
                                ${customizeExperienceText}
                            </h4>
                            <p style="color: #aaa; font-size: .72rem; margin: 0;">
                                ${txtUsa}${approxTimeText}
                            </p>
                        </div>
                        <div id="otg-fase-1">
                            ...
                            <button onclick="OTG_SENSORIAL.displayBrandSpecificQuestions()" style="width: 100%; background: #2e7d32; border: none; color: #fff; padding: 14px; border-radius: 6px; font-weight: bold; cursor: pointer; text-transform: uppercase; font-size: .8rem; letter-spacing: .5px;">
                                ${continueBtnText}
                            </button>
                        </div>
                ...
                `;
                this.seleccionadas = []; // Clear previous selections
                this.hablar(selectServiceText);
            },

            seleccionarMarca(el, marca) {
                // Ensure only one brand can be selected at a time
                const currentlyActive = document.querySelector(".otg-card-logo.active");
                if (currentlyActive && currentlyActive !== el) {
                    currentlyActive.classList.remove("active");
                    this.seleccionadas = []; // Clear previous selection
                }
            ...
            displayBrandSpecificQuestions() { // Renamed from activarFaseTrivia
                if (!this.seleccionadas.length) {
                    KERNEL.hablar(KERNEL.idiomaActual === 'es' ? "Por favor, selecciona al menos una opción." : "Please select at least one option.");
                    return;
                }
                ...
                let selectedBrand = this.seleccionadas[0]; // Get the first (and only) selected brand
                let questions = this.brandSpecificQuestions[selectedBrand] || this.brandSpecificQuestions["GENERIC"];
                
                let brandIntroText = KERNEL.idiomaActual === 'es' ? `Has seleccionado: ${selectedBrand}. Ahora, elige la pregunta que mejor describe lo que sientes.` : `You selected: ${selectedBrand}. Now, choose the question that best describes what you feel.`;
                let skipQuestionsText = KERNEL.idiomaActual === 'es' ? "O saltar preguntas y continuar" : "Or skip questions and continue";
                ...
                f2.innerHTML = `
                    <div style="background: #111; border: 1px solid #222; padding: 15px; border-radius: 8px; margin-top: 10px;">
                        ...
                        <div style="font-size: 1rem; font-weight: bold; line-height: 1.45; margin: 5px 0 15px; color: #fff;">
                            ${questions.map((q, i) => `
                                <button class="otg-btn-opt" onclick="OTG_SENSORIAL.handleBrandQuestionClick('${selectedBrand}', '${q[KERNEL.idiomaActual]}')">
                                    ${i + 1}. ${q[KERNEL.idiomaActual]}
                                </button>
                            `).join("")}
                        </div>
                        <button onclick="OTG_SENSORIAL.skipBrandQuestions('${selectedBrand}')" class="otg-btn-opt" style="border-style: dashed; opacity: 0.7;">
                            ${skipQuestionsText}
                        </button>
                    </div>
                `;
                KERNEL.hablar(brandIntroText);
            },

            handleBrandQuestionClick(selectedBrand, questionText) {
                // This function sets the desahogo and transitions to the KERNEL main form
                this.finalizeOasisFlow(selectedBrand, questionText);
            },

            skipBrandQuestions(selectedBrand) {
                // This function transitions directly to KERNEL main form, setting only the brand
                this.finalizeOasisFlow(selectedBrand, "");
            },

            finalizeOasisFlow(selectedBrand = "", optionalQuestionText = "") { // Renamed from cerrarOasisYDarPasoAAppBase
                let oasisDiv = document.getElementById("otg-oasis-entretenimiento");
                if (oasisDiv) oasisDiv.classList.add("hidden");
                document.body.style.overflow = "auto"; // Re-enable scrolling

                // Show the main KERNEL wrapper form
                document.getElementById('wrapper-form').classList.remove('hidden');
               
                // Make floating buttons visible
                ...
                // Pre-fill desahogo if a brand or question was selected
                if (selectedBrand) {
                    let desahogoInput = document.getElementById('inp-text-libre');
                    if (desahogoInput) {
                        desahogoInput.value = optionalQuestionText ? `"${selectedBrand}": ${optionalQuestionText}` : selectedBrand;
                    }
                }

                // Re-initialize KERNEL's form interactions now that it's visible
                KERNEL.cambiarIdioma(KERNEL.idiomaActual); // Ensure form is in correct language and questions are loaded
                KERNEL.iniciarMonitoreoInaccion(); // Starts inactivity monitor for KERNEL
                KERNEL.activarBotonMandoLibreInicial(); // Activates free writing box
               
                let powerBtn = document.getElementById("otg-btn-power");
                if (powerBtn) powerBtn.classList.remove("hidden"); // OTG_SENSORIAL power button
               
                this.seleccionadas = []; // Clear selected brands for next use
                console.log("OPEN THAN GO main interface initiated.");
            },

            apagarSistemaTotal() {
                // Clear all timers and reset everything from KERNEL's perspective
                KERNEL.destruirYReiniciar(); // This will clear all localStorage and reload the page
            },

            forzarCierre15Minutos() {
                // This is the fallback if user is stuck in OTG_SENSORIAL flow for too long.
                // It's a hard reset, similar to KERNEL's, but only for the old OTG_SENSORIAL's purpose.
                // Given KERNEL's global timer, we might not need this here, but keep it for robustness.
                let m = document.getElementById("otg-oasis-entretenimiento");
                ...
                `;
                // Also clear KERNEL timers if they were somehow running
                KERNEL.destruirYReiniciar(); // This will clear all localStorage and reload the page
            }
        };

    //==========================================================================================
    // KERNEL CLÍNICO PRINCIPAL DE OPEN THAN GO (SECCIÓN MAESTRA)
    //==========================================================================================
    const KERNEL = {
        timerInaccion: null,
        timerEnfocado: null,
        ...
        "IMAGENES_CARRUSEL": {
            ...
            "estresado": [ // Images to soothe, provide expansive views, clear skies, natural order
                ...
                "static/images/stressed_19_mountain_lake.jpg", "static/images/stressed_20_old_growth_forest.jpg", "static/images/stressed_21_rolling_hills.jpg",
    ...
    "CATALOGO_RETOS_EN": [
      ...
      {"id": 210, "titulo_en": "THE VISUAL REST CHALLENGE", "descripcion_en": "Look for a spot or an object that is very far away from you through the window. Keep your eyes on that place fixedly for two minutes to rest your eyes from screens completely. Expand your visual horizon, relax your eyelids, and allow your thoughts to flow in total stillness.", "img": "nature_sound.svg"}
         ...
         * This now acts as the entry point from OTG_SENSORIAL back to KERNEL's main form.
         */
        despertarInicial() {
            const welcomeScreen = document.getElementById('pantalla-bienvenida');
            if (welcomeScreen) welcomeScreen.style.display = 'none';
           
            // KERNEL's main form and floating buttons should be hidden initially and shown by OTG_SENSORIAL.finalizeOasisFlow
            ...
            document.getElementById('otg-btn-power').classList.add('hidden'); // Hide OTG_SENSORIAL power button initially

            this.cambiarIdioma(this.idiomaActual); // Set language and update form texts
           
            // Instead of showing wrapper-form directly, let OTG_SENSORIAL handle the initial flow
            // This function is now the *true* entry after the paywall
            window.OTG_SENSORIAL.abrirOasisOcio(); // This will display the company selection
           
            this.horaInicioSesionAbsoluta = Date.now();
            // The rest of KERNEL setup (inyectarBloquePreguntas, iniciarMonitoreoInaccion, activarBotonMandoLibreInicial)
            // will now be called by OTG_SENSORIAL.finalizeOasisFlow() after the initial company/question flow.
        },
        ...
        */
        cambiarIdioma(lang) {
            ...
            document.getElementById('inp-zip').placeholder = lang === 'es' ? "Ej: 33167" : "Ex: 90210";
     ...
     localStorage.setItem("otg_historial_casa", JSON.stringify(this.historialCasa));
        ...
        iniciarRelojEnfocadoCasa(container, t) {
            ...
            for (let i = imagesForMente.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [imagesForMente[i], imagesForMente[j]] = [imagesForMates[j], imagesForMente[i]];
            }
        ...
        reiniciarExperiencia() {
            ...
            if (wrapperForm) wrapperForm.classList.add('hidden'); // Hide main form, go back to OTG_SENSORIAL
            if (inpTextLibre) inpTextLibre.value = "";
           
            // Go back to the OTG_SENSORIAL initial selection flow
            window.OTG_SENSORIAL.abrirOasisOcio();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // Initialize OTG_SENSORIAL first, then KERNEL
        if (typeof window.OTG_SENSORIAL !== 'undefined' && window.OTG_SENSORIAL.init) {
            window.OTG_SENSORIAL.init();
            console.log("OTG_SENSORIAL inicializado.");
        }

        if (typeof KERNEL !== 'undefined' && KERNEL.init) {
            KERNEL.init();
            console.log("KERNEL principal inicializado.");
        }
    });
