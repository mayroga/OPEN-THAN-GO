// ==========================================================================================
// OPEN THAN GO SYSTEM - Módulo Especial Adaptado (Frontend Production Clone v6.0)
...
(function () {
        ...
        container.innerHTML = `
            <div class="cinematic-module-container">
                ...
                <div class="cortina-desplegable-premium" style="background:#040404; padding:20px; border-radius:14px; border:1px solid #161616; margin: 15px 0;">
                    <label style="display:block; color: #00bcd4; font-weight:900; margin-bottom:15px; font-size:0.85rem; text-transform:uppercase; letter-spacing:1px;">\${t.lblKeywords}</label>
                    ...
                    <div style="margin-top:25px; display:flex; gap:14px; justify-content:center; align-items:center;">
                        <button id="btn-mic-especial" class="pill-action-trigger" style="color:#fff; background:#0b0b0b; border:1px solid #222; padding:10px 20px; border-radius:40px; cursor:pointer;">🎙️ \${t.btnMicGrabar}</button>
                ...
                <button id="btn-procesar-especial" class="action-btn-fatal-premium" style="width:100%; background:linear-gradient(135deg, #4caf50 0%, #1b5e20 100%); color:#fff; padding:18px; font-weight:900; border-radius:12px; border:none; cursor:pointer; text-transform:uppercase;">\${t.btnProcesar}</button>
    ...
    async generarReporteBienestar() {
        ...
        try {
            ...
            if (window.KERNEL?.hablar) {
                window.KERNEL.hablar(lang === 'es' ? "Reporte descriptivo generado." : "Reporte generated.");
            }
        } catch (e) {
