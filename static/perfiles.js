...
ejecutarVozBienvenida() {
    ...
    window.KERNEL.hablar(msgs[this.perfilSeleccionado]);
        ...
        renderizarInterfazEspecial() {
            ...
            if (!container) return;
@@ -188,24 +183,23 @@ ejecutarVozBienvenida() {
...
@@ -215,7 +209,7 @@ ejecutarVozBienvenida() {
                    </div>
...
@@ -225,234 +219,266 @@ ejecutarVozBienvenida() {
            ...
            if (fileInput) {
                fileInput.onchange = (e) => {
                    ...
                    reader.onload = (event) => {
                        const buffer = event.target.result;
                        const chunkStr = new TextDecoder("utf-8").decode(new Uint8Array(buffer).slice(0, 7000));
                        this.textoPdfExtraido = "Flujo real analizado del archivo. Extraídas variables biológicas para el motor.";
...
detenerGraboHardware(btn, t) {
    ...
    if (this.recognitionInstance) this.recognitionInstance.stop();
        ...
        detenerGraboHardware(btn, t) {
            ...
            btn.style.background = "#0b0b0b";
            btn.style.borderColor = "#222";
            btn.innerText = "🎙️ " + t.btnMicGrabar;
            if (this.recognitionInstance) this.recognitionInstance.stop();
        },
    ...
    try {
        ...
        async generarReporteBienestar() {
            ...
            try {
                ...
                // Corrección del bloque HTML: Etiquetas estructuradas y estilos CSS limpios
                wrapperReporte.innerHTML = `
                    <div class="reporte-premium-box" style="margin-top:25px; border-top:1px dashed #222; padding-top:20px; text-align:left;">
                        ...
                        <h4 style="color:#fff; margin:0 0 5px 0; font-size:0.9rem; text-transform:uppercase;">Pautas de Acompañamiento:</h4>
                        <p style="font-size:0.85rem; color:#ccc; margin:0; line-height:1.4;">${data.observaciones_finales}</p>
