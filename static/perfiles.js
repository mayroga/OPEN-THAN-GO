...
// Company: May Roga LLC - Version: 5.5.0 - Interfaz Letal contra el Agobio en Cortina
    ...
    const PERFILES_ESPECIALES = {
        ...
        inyectarEstilosCinematicos() {
            ...
            css.textContent = `
                ...
                .portal-card-premium.active { background: #111; border-color: #00bcd4; box-shadow: 0 0 15px rgba(0,188,212,0.1); }
                ...
                .badge-keyword-fatal.selected { background: #4caf50; color: #fff; border-color: transparent; box-shadow: 0 0 15px rgba(76,175,80,0.5); }
...
🎖️ Portal Veteranos de Guerra
⚡
👵 Portal Adultos Mayores
⚡
💼 Portal Servidores Públicos
⚡
Sintonizadores Somáticos Interactivos:
🎙️ Sintonizar por Voz
📂 Escanear PDF Estructural
⚡ Lanzar Desconexión Letal e Inmediata
📋 Solicitar Reporte Descriptivo`;
...
card.onclick = () => {
container.querySelectorAll(".portal-card-premium")
...
const chunkStr = new TextDecoder("utf-8")
.decode(new Uint8Array(buffer).slice(0,7000));
this.textoPdfExtraido =
"Flujo real binario analizado. Variables integradas al motor.";
if (window.KERNEL?.hablar) {
window.KERNEL.hablar(
window.KERNEL.idiomaActual === 'es'
? "Documento sincronizado."
: "Documento synchronized."
);
}
alert(
this.TEXTOS[
...
if (btnMic)
btnMic.onclick = () =>
this.gestionarFlujoMicrofono(btnMic);
document.getElementById("btn-procesar-especial").onclick =
() => this.ejecutarMandoEspecial();
document.getElementById("btn-reporte-especial").onclick =
() => this.generarReporteBienestar();
},
async cargarKeywordsPerfil() {
const box =
document.getElementById("box-keywords-flotantes");
if (!box) return;
box.innerHTML = "";
const lang =
window.KERNEL?.idiomaActual || "es";
...
const res =
await fetch(
/api/perfiles-especiales/config?perfil=${this.perfilSeleccionado}&lang=${lang}
);
...
if (window.KERNEL?.hablar)
window.KERNEL.hablar(kw.toLowerCase());
} else {
this.keywordsSeleccionadas =
...
const textoVoz =
event.results[currentIdx][0].transcript;
const hdnInput =
document.getElementById("txt-input-especial");
if (hdnInput && textoVoz) {
hdnInput.value =
(hdnInput.value + " " + textoVoz).trim();
if (window.KERNEL?.hablar)
window.KERNEL.hablar(textoVoz);
}
};
this.recognitionInstance.onerror =
...
document.getElementById("txt-input-especial");
const txtInput =
hdnInput ? hdnInput.value.trim() : "";
const lang =
window.KERNEL?.idiomaActual || "es";
if (
txtInput.length === 0 &&
this.keywordsSeleccionadas.length === 0 &&
this.textoPdfExtraido.length === 0
) {
...
data.calidez_humana;
this.frasesRespiracionActuales =
data.frases_respiracion;
...
const linkMaps =
containerInteractive.querySelector("a[href*='maps']");
if (linkMaps && data.misiones && data.misiones[0]) {
const iframeMaps =
document.createElement("iframe");
iframeMaps.style =
"width:100%; height:320px; border:1px solid #1a1a1a; border-radius:12px; margin-top:15px;";
iframeMaps.src =
data.misiones[0].destino_coordenadas_gps;
linkMaps.parentNode.insertBefore(
iframeMaps,
linkMaps.nextSibling
);
linkMaps.style.display = "none";
}
const linkYT =
containerInteractive.querySelector("a[href*='youtube']");
if (linkYT && data.misiones && data.misiones[0]) {
const iframeYT =
document.createElement("iframe");
iframeYT.style =
"width:100%; height:240px; border:1px solid #1a1a1a; border-radius:12px; margin-top:15px;";
iframeYT.src =
data.misiones[0].enlace_youtube;
iframeYT.allow =
"autoplay; encrypted-media";
linkYT.parentNode.insertBefore(
iframeYT,
linkYT.nextSibling
);
...
console.error(
"Fallo crítico despachando mando:",
e
);
this.activo = false;
this.alternarVisibilidadPaneles();
...
infoCompartida.push("Mapeo de documento.");
const payload = {
perfil:this.perfilSeleccionado,
lang:lang,
...
try {
const res = await fetch(
"/api/perfiles-especiales/reporte",
{
method:"POST",
headers:{
"Content-Type":"application/json"
...
<h2 style="color:var(--cyan-inhale,#00bcd4);font-size:1.1rem;font-weight:900;text-transform:uppercase;margin:0 0 10px 0;">
${data.titulo}
</h2>
<p style="font-size:0.85rem;color:#888;line-height:1.4;margin-bottom:15px;">
${data.resumen_descriptivo}
</p>
<h4 style="color:#fff;margin:0 0 5px 0;font-size:.9rem;text-transform:uppercase;">
Recorrido Concluido:
</h4>
<ul style="padding-left:15px;margin:0 0 15px 0;font-size:.85rem;color:#ccc;">
${data.recorrido_realizado.map(r=>`<li>${r}</li>`).join("")}
</ul>
<h4 style="color:#fff;margin:0 0 5px 0;font-size:.9rem;text-transform:uppercase;">
Evaluación de Autocuidado Comercial:
</h4>
<p>
${data.observaciones_finales}
</p>
<div class="nota-legal-reporte">
${data.nota_legal}
</div>
</div>
`;
...
window.KERNEL.hablar(
lang === "es"
? "Reporte descriptivo de acompañamiento generado."
: "Descriptive report generated."
);
}
} catch (e) {
console.error(
...
const originalRelojCasa =
window.KERNEL.iniciarRelojEnfocadoCasa;
window.KERNEL.iniciarRelojEnfocadoCasa=function(){
if(
PERFILES_ESPECIALES.activo &&
PERFILES_ESPECIALES.frasesRespiracionActuales
){
if(window.KERNEL.hablar)
window.KERNEL.hablar(
PERFILES_ESPECIALES.frasesRespiracionActuales.antes
);
}
...
if(window.KERNEL.hablar)
window.KERNEL.hablar(
PERFILES_ESPECIALES.frasesRespiracionActuales.despues
);
},900000);
}
};
